  ...
  size_t current_alignment)
{
  ...
  if (is_plain) {
    ...
    using DataType = xarm_msgs__srv__GetDigitalIO_Request;
    is_plain =
      (
...
static size_t _GetDigitalIO_Request__max_serialized_size(char & bounds_info)
{
  bool full_bounded;
  bool is_plain;
  size_t ret_val;

  ret_val = max_serialized_size_xarm_msgs__srv__GetDigitalIO_Request(
  ...
  void * untyped_ros_message)
{
  ...
  // Field name: digitals
  {
    ...
    if (!rosidl_runtime_c__int16__Sequence__init(&ros_message->digitals, size)) {
      fprintf(stderr, "failed to create array for field 'digitals'");
      return false;
    }
    auto array_ptr = ros_message->digitals.data;
  ...
  size_t current_alignment)
{
  ...
  if (is_plain) {
    ...
    // We still need to check that the in-memory alignment
    // is the same as the CDR mandated alignment.
    using DataType = xarm_msgs__srv__GetDigitalIO_Response;
    is_plain =
      (
      offsetof(DataType, digitals) +
      last_member_size
      ) == ret_val;
  }
...
static size_t _GetDigitalIO_Response__max_serialized_size(char & bounds_info)
{
  bool full_bounded;
  bool is_plain;
  size_t ret_val;

  ret_val = max_serialized_size_xarm_msgs__srv__GetDigitalIO_Response(
    full_bounded, is_plain, 0);
