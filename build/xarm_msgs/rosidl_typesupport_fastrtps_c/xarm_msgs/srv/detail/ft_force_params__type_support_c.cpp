  ...
  void * untyped_ros_message)
{
  ...
  // Field name: xe_limit
  {
    ...
    if (!rosidl_runtime_c__float__Sequence__init(&ros_message->xe_limit, size)) {
      fprintf(stderr, "failed to create array for field 'xe_limit'");
      return false;
    }
    auto array_ptr = ros_message->xe_limit.data;
    cdr.deserializeArray(array_ptr, size);
  ...
  size_t current_alignment)
{
  ...
  if (is_plain) {
    ...
    using DataType = xarm_msgs__srv__FtForceParams_Request;
    is_plain =
      (
...
static size_t _FtForceParams_Request__max_serialized_size(char & bounds_info)
{
  bool full_bounded;
  bool is_plain;
  size_t ret_val;

  ret_val = max_serialized_size_xarm_msgs__srv__FtForceParams_Request(
  ...
  size_t current_alignment)
{
  ...
  if (is_plain) {
    ...
    // We still need to check that the in-memory alignment
    // is the same as the CDR mandated alignment.
    using DataType = xarm_msgs__srv__FtForceParams_Response;
    is_plain =
      (
...
static size_t _FtForceParams_Response__max_serialized_size(char & bounds_info)
{
  bool full_bounded;
  bool is_plain;
  size_t ret_val;

  ret_val = max_serialized_size_xarm_msgs__srv__FtForceParams_Response(
    full_bounded, is_plain, 0);
