  ...
  size_t current_alignment)
{
  ...
  if (is_plain) {
    ...
    using DataType = xarm_msgs::srv::GetInt32ByType_Request;
    is_plain =
      (
...
static size_t _GetInt32ByType_Request__max_serialized_size(char & bounds_info)
{
  ...
  ret_val = max_serialized_size_GetInt32ByType_Request(full_bounded, is_plain, 0);
  ...
  size_t current_alignment)
{
  ...
  if (is_plain) {
    ...
    // We still need to check that the in-memory alignment
    // is the same as the CDR mandated alignment.
    using DataType = xarm_msgs::srv::GetInt32ByType_Response;
    is_plain =
      (
...
static size_t _GetInt32ByType_Response__max_serialized_size(char & bounds_info)
{
  ...
  ret_val = max_serialized_size_GetInt32ByType_Response(full_bounded, is_plain, 0);
